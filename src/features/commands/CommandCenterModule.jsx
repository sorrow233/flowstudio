...
const CommandCenterModule = () => {
    ...
    const [newTag, setNewTag] = useState({ label: '', value: '' }); // Temp state for adding a tag
    ...
    const handleAddTag = () => {
        ...
        // If value is empty, it defaults to parent content in logic, but let's enforce or allow it.
        // User wants specific text, so value should probably be required or default to main content.
        // Let's require it for clarity if they are making a tag.
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {newCmd.type === 'utility' && (
                                            <div className="space-y-3 pt-2 border-t border-gray-100">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tags & Variants</span>
                                                </div>
                                                
                                                <div className="flex gap-2">
                                                    <input 
                                                        className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-sm outline-none focus:bg-white focus:border-emerald-200 transition-all"
                                                        placeholder="Tag Label (e.g. 'Dev')"
                                                        value={newTag.label}
                                                        onChange={e => setNewTag({...newTag, label: e.target.value})}
                                                    />
                                                    <input 
                                                        className="flex-[2] bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-sm outline-none focus:bg-white focus:border-emerald-200 transition-all font-mono text-gray-600"
                                                        placeholder="Specific Content (Leave empty to use main)"
                                                        value={newTag.value}
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-4 py-2 bg-gray-900 text-white rounded-xl text-xs font-medium hover:bg-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                                    >
                                                        Add
                                                    </button>
                                                </div>
                                                ...
                                                {(newCmd.tags && newCmd.tags.length > 0) && (
                                                    <div className="flex flex-wrap gap-2 mt-2">
                                                        {newCmd.tags.map(tag => (
                                                            <div key={tag.id} className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-medium border border-emerald-100">
                                                                <Tag size={12} />
                                                                <span>{tag.label}</span>
                                                                <span className="text-emerald-400">|</span>
                                                                <span className="font-mono opacity-70 max-w-[150px] truncate">{tag.value}</span>
                                                                <button 
                                                                    onClick={() => handleRemoveTag(tag.id)}
                                                                    className="ml-1 p-0.5 hover:bg-emerald-100 rounded-full text-emerald-500 transition-colors"
                                                                >
                                                                    <X size={12} />
                                                                </button>
                                                            </div>
                        ...
                        >
                            ...
                            >
                                ...
                                {(cmd.tags && cmd.tags.length > 0) && (
                                    <div className="flex flex-wrap gap-2 px-4 pb-4 pl-[4.5rem] mt-2 w-full">
                                        {cmd.tags.map(tag => (
                                            <button
                                                key={tag.id}
                                                onClick={(e) => {
                                                ...
                                                className="group/tag flex items-center gap-2 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-xs font-medium border border-emerald-100 hover:border-emerald-200 transition-all relative overflow-hidden"
                                                title={tag.value}
                                            >
