...
const CHUNK_RELOAD_STORAGE_KEY = 'flowstudio_chunk_reload_fingerprint';
...
function getRecoveryFingerprint(buildId, contextName) {
    if (typeof window === 'undefined') {
        return `${buildId || 'unknown'}::${contextName || 'unknown'}::server`;
    }

    const currentUrl = new URL(window.location.href);
    currentUrl.searchParams.delete('reload');
    const search = currentUrl.searchParams.toString();
    const stablePath = `${currentUrl.pathname}${search ? `?${search}` : ''}${currentUrl.hash}`;

    return `${buildId || 'unknown'}::${contextName || 'unknown'}::${stablePath}`;
}
